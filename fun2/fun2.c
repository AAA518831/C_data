...
int main(void)
{
	...
	for (int i = 1; i <= 3; i++) // 3번 반복
	{
		...
		scanf("%d", &sel); //&는 주소를 의미함.


		if (sel == 1) res1 = sum(a, b); // 함수 호출 // a, b 인수 보냄 
		// sum 함수 호출하여 반환(return)된 값을 res1에 저장
		...
		{
			prn1(res1); //함수호출(정수형 결과를 출력하는 함수)
			break;
		}
		else if (sel == 4)
		{
			prn2(res2); //함수호출(실수형 결과 출력)
			break;
		}
		else printf("입력 오류. 다시 입력하세요.\n");
	}

		return 0;
}

int sum(int x, int y) //매개변수로 받음 a -> x, b -> y
                  //매개변수는 형(정수형)을 선언하면서 변수 기록
	              //인수와 매개변수는 개수와 형이 같아야함
{
	int k;
	k = x + y;
	return k; 
...
void prn2(double result2) // res2 -> result2(정수형 받음)
// 반환값이 없이 돌아감(void 함수형 처리)
{
	printf("결과는 %.2lf", result2); //출력 완료 //float는 double은 lf
	return; // 생략 가능
}
